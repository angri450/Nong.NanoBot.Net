...
[English README](README.md) · [Releases](https://github.com/angri450/Nong.NanoBot.Net/releases) · [GitHub](https://github.com/angri450/Nong.NanoBot.Net)
...
Nong.NanoBot.Net 是一个独立的 .NET 8 个人 Agent 运行时，面向本地自动化、工具执行、记忆、聊天网关、MCP 和多 Provider LLM 路由。它的目标不是做一个演示脚本，而是给 C# 生态提供一个紧凑但具备工程形态的 local-first Agent 底座。
...
`Nanobot.Web` 是本地浏览器工作台，用来可视化使用 Nong.NanoBot.Net runtime。它通过 ASP.NET Core 后端和静态 HTML/CSS/JS 前端提供 runtime 状态、流式 session 聊天、WebUI 会话持久化、工作区文件浏览和预览、SSE 工具事件详情以及记忆预览。

```bash
dotnet run --project Nanobot.Web --urls http://127.0.0.1:8788
```

CLI wrapper 会启动同一个 WebUI runtime，也是 Windows 安装包使用的入口：
...
Nong.NanoBot.Net 有实用安全护栏，但不是完整的敌意用户沙箱。
