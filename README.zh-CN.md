...
`Nanobot.Web` 是本地浏览器工作台，用来可视化使用 NanoBot.net runtime。它通过 ASP.NET Core 后端和静态 HTML/CSS/JS 前端提供 runtime 状态、流式 session 聊天、WebUI 会话持久化、工作区文件浏览和预览、SSE 工具事件详情以及记忆预览。
...
NanoBot.net 现在可以打成 Windows x64 MSI。这个 MSI 不使用 WebView2、Electron，也不会塞一个常驻浏览器壳；它安装的是自包含 CLI 和 WebUI runtime，创建开始菜单快捷方式，并把 `nanobot.exe` 加到当前用户 PATH。GroundPA-Toolkit 和 Nong 仍然走后续 plugin/bootstrap 机制安装，不打包进 MSI 主负载。
