...
本轮 P6 的目标是先把外部智能体项目研究清楚，再决定 Nong.NanoBot.Net 后续吸收什么。一个问题不能两头都是谜团：NanoBot 继续作为主线，但 P6 不直接施工新功能，先做横向调研和架构取舍。
...
- 能分发 MSI
- 能和 Nong / Nong.Toolkit.Net 走 plugin/bootstrap
...
- GenericAgent.net：Python 极简路线优秀，但和 .NET/Nong/Nong.Toolkit.Net 统一目标不一致。
...
它对 NanoBot 的 product direction 很有价值，尤其是 Nong.Toolkit.Net 未来要做多 workspace、多任务、白盒记忆和后台运行。
...
它不是最适合直接做 NanoBot runtime 的项目，但它的“最小工具集”和“自进化 skill”非常适合 Nong.Toolkit.Net 的长期路线。
...
- 科研任务很强，但 NanoBot 当前更偏本地 coding/Nong.Toolkit.Net runtime。
...
- 体系很重，容易把 NanoBot 做成框架集成项目，而不是轻量 Nong.Toolkit.Net runtime。
...
```text
Nong.NanoBot.Net = .NET 8 Nong.Toolkit.Net runtime host
