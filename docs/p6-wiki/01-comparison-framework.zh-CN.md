...
本框架用于判断外部项目哪些能力值得进入 Nong.NanoBot.Net，避免凭 UI 好不好看或模型能不能白嫖来决定主线。
...
| 记忆体系 | 白盒记忆、Dream/归纳、可编辑/可回滚、workspace 隔离 | 决定 Nong.Toolkit.Net 是否可持续工作 |
| Tools/MCP/Skills | 工具注册、MCP transport、skill/plugin manifest、marketplace | 决定 Nong/Nong.Toolkit.Net 怎么接入 |
...
1. Nong.NanoBot.Net 默认保持主线，除非有项目同时满足技术栈、许可、产品目标和本地优先。
