# Nong.NanoBot.Net 开发施工方案

本文是 Nong.NanoBot.Net 的主施工方案。DeepSeek-GUI.net、CodeBuddy、DeepSeek-TUI、DeepSeek-GUI 等项目只作为 UI/交互/信息架构参考；真正要开发和沉淀能力的主仓库是 `Nong.NanoBot.Net`。

## 方向判断

Nong.NanoBot.Net 继续作为 Nong.Toolkit.Net / Nong.Cli.Net agent-runtime 层主线，目标是做成一个 .NET 8、本地优先、可视化可用、工具调用可审计的 Nong.Toolkit.Net host。

这里的关键判断是：NanoBot 出场不直接打包 Nong.Toolkit.Net / Nong 的完整负载，但 NanoBot 必须内置一套“技能包 / plugin”安装部署方案。用户拿到 NanoBot 后，可以像 Claude Code plugin marketplace 那样，从带 `plugin.json` 的仓库一键安装、部署、升级能力包。Nong.Toolkit.Net 和 Nong 都通过这套机制接入，而不是和 NanoBot 发行包绑死。
...
- Nong.Toolkit.Net / Nong：不随 NanoBot 二进制打包完整负载，由技能包机制在初始化或运行时安装、更新和健康检查
...
Nanobot.Web / Runtime Host
  ...
  plugin / Nong.Toolkit.Net bootstrap status and background deployment
        |
        v
Nanobot.Core
  Agent loop
  ...
  Nong.Toolkit.Net installer/updater
        |
        v
Local workspace + installed skill packs + installed Nong.Toolkit.Net toolkit + external model/tool APIs
```
...
NanoBot 要内置一套通用技能包方案，而不是只给 Nong.Toolkit.Net 写死安装逻辑。目标是复用并融合 Claude Code plugin marketplace 的体验：只要一个仓库带 `plugin.json`，NanoBot 就能识别、安装、部署和更新。
...
```json
{
  ...
  "publisher": "angri450",
  "version": "0.1.0",
  "displayName": "Nong.Toolkit.Net",
  "description": "Nong.Toolkit.Net deterministic tool skills for NanoBot.",
  "install": {
    "commands": []
  },
  "tools": [
...
### Nong.Toolkit.Net Bootstrap

NanoBot 的 Nong.Toolkit.Net 策略是“出场不自带，安装能力自带”：

- NanoBot 发行包不直接塞入完整 Nong.Toolkit.Net / Nong 包。
...
- 模型看到的是稳定的 `run_nong` / Nong.Toolkit.Net capability，不需要知道安装过程细节。

Nong.Toolkit.Net 是 NanoBot plugin 机制的第一块核心样板。命令可以保留 Nong.Toolkit.Net 语义糖，但底层走同一套 plugin 安装系统：
...
其中 `nong-toolkit/` 可以视为 plugin 状态的聚合视图，真实安装记录仍应回写 `plugins/installed.json`，避免 Nong.Toolkit.Net 形成一条孤立的安装体系。

第一阶段可以先实现最小闭环：

1. 检测本机是否已有 `nong` 命令。
2. 若没有，记录 `missing` 状态并给出安装入口。
3. 支持从配置的 Nong.Toolkit.Net 源安装或更新。
...
### P4：Plugin / Nong.Toolkit.Net Bootstrap 与 Agent 操作面板
...
- Nong.Toolkit.Net 作为第一块核心 plugin 样板
- Nong.Toolkit.Net marketplace/source 配置
- Nong 安装、更新、部署、健康检查
- runtime 启动后后台继续部署 Nong 包
- WebUI 展示 Nong.Toolkit.Net/Nong.Cli.Net ready、installing、failed、update available 状态
...
- Nong.Toolkit.Net / Nong 继续通过 plugin 机制安装，不塞进 MSI 主负载。
...
GET  /api/nong-toolkit/status
POST /api/nong-toolkit/install
POST /api/nong-toolkit/update
GET  /api/nong-toolkit/capabilities
GET  /api/memory
...
AtomCode.net 是 GitCode 免费模型通道的重要参考线，但它不是 Nong.NanoBot.Net 的新主线，也不应成为 NanoBot 的运行时依赖。NanoBot 要吸收的是“GitCode 登录、CodingPlan 领取、模型目录同步、provider 自动注册、WebUI 控制面”这条产品链路。
...
3. 固定 Nong.NanoBot.Net 不换主线，只吸收外部项目的局部设计。
...
4. 增加 plugin / Nong.Toolkit.Net / Nong.Cli.Net 状态 API：ready、missing、installing、failed、update available。
5. 在 WebUI 展示 plugin、Nong.Toolkit.Net、Nong 的安装进度和健康状态。
...
- 带 `plugin.json` 的仓库安装
- plugin 安装中、失败、更新状态
- Nong.Toolkit.Net/Nong.Cli.Net 缺失时的安装入口
- Nong.Toolkit.Net/Nong.Cli.Net 安装中状态
- `nong commands --json` capability discovery
...
- 主施工仓库是 `Nong.NanoBot.Net`。
- 外部 GUI 仓库只做参考和对比，不承载 NanoBot 的主方案。
- Nong.Toolkit.Net / Nong 不作为完整负载打包进 NanoBot 发行物；NanoBot 内置的是 plugin marketplace、安装、部署、升级、检测和运行桥接能力。
- Nong.Toolkit.Net、Nong 和其他技能包应独立更新，NanoBot 只负责安装编排、状态管理、capability catalog 和 runtime bridge。
